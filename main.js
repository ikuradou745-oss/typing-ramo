...
function updateHUD() {
    ...
    document.getElementById("current-xp").innerText = UserState.experiencePoints;
    
    const requiredXP = calculateRequiredXP(UserState.level);
    document.getElementById("required-xp").innerText = requiredXP;
    document.getElementById("xp-bar-fill").style.width = `${Math.min((UserState.experiencePoints / requiredXP) * 100, 100)}%`;
...
function initializeFriendSystem() {
    ...
    onValue(ref(firebaseDatabase, `friendRequests/${UserState.uid}`), (snapshot) => {
        ...
        document.getElementById("request-badge").innerText = count;
    });
...
async function updateTaskProgress(score, combo) {
    ...
    const check = async (tasks, type, currentVal, targetId) => {
        for (let t of tasks) {
...
let gameActive = false, score = 0, combo = 0, maxCombo = 0, timeLeft = 60, timer = null, currentWord = null, charIdx = 0;
...
function renderWord() {
    document.getElementById("japanese-word").innerText = currentWord.text;
    document.getElementById("romaji-word").innerHTML = currentWord.romaji.split('').map((c, i) => 
...
const togglePanel = (id, open) => {
    ...
    if (open) { p.classList.remove("hidden"); p.classList.add("open"); ov.classList.remove("hidden"); }
    else { p.classList.remove("open"); setTimeout(() => p.classList.add("hidden"), 400); ov.classList.add("hidden"); }
};

document.getElementById("profile-button").onclick = () => togglePanel("profile-panel", true);
document.getElementById("close-profile-button").onclick = () => togglePanel("profile-panel", false);
document.getElementById("task-button").onclick = () => togglePanel("task-panel", true);
document.getElementById("close-task-button").onclick = () => togglePanel("task-panel", false);

// 名前変更
document.getElementById("change-name-button").onclick = async () => {
...
// ゲーム開始
document.getElementById("start-offline-play-button").onclick = () => {
    document.getElementById("start-offline-play-button").classList.add("hidden");
...
window.addEventListener("keydown", (e) => {
    ...
    if (e.key.toLowerCase() === target) {
        ...
        score += 5 * combo;
        renderWord(); updateGameUI();
        if (charIdx >= currentWord.romaji.length) nextWord(document.querySelector(".difficulty-button").dataset.difficulty);
    } else {
        combo = 0; updateGameUI();
    }
});
